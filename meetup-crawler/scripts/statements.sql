...
CREATE TABLE IF NOT EXISTS meetup_event (utc_offset BIGINT, rsvp_limit BIGINT, headcount BIGINT, visibility VARCHAR, waitlist_count BIGINT, created BIGINT, maybe_rsvp_count BIGINT, description VARCHAR, how_to_find_us VARCHAR, event_url VARCHAR, yes_rsvp_count BIGINT, name VARCHAR, id VARCHAR, time BIGINT, duration BIGINT, updated BIGINT, photo_url VARCHAR, venue_country VARCHAR, venue_localized_country_name VARCHAR, venue_city VARCHAR, venue_address_1 VARCHAR, venue_name VARCHAR, venue_lon NUMERIC, venue_id BIGINT, venue_lat NUMERIC, venue_repinned BOOLEAN, rating_count BIGINT, rating_average BIGINT, group_join_mode VARCHAR, group_created BIGINT, group_name VARCHAR, group_group_lon NUMERIC, group_id BIGINT, group_urlname VARCHAR, group_group_lat NUMERIC, group_who VARCHAR, last_updated_at BIGINT, raw_json JSON NOT NULL);
